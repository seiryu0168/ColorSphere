...
namespace Direct3D
{
	...
	IDXGIFactory1* pFactory;									//ファクトリー
	IDXGIAdapter* pAdapter;										//アダプター
...
HRESULT Direct3D::Initialize(int winW, int winH, HWND hWnd)
{
	...
	scDesc.BufferUsage = DXGI_USAGE_RENDER_TARGET_OUTPUT| DXGI_USAGE_SHADER_INPUT;	//バックバッファの使い道＝画面に描画するために
	...
	hr=D3D11CreateDevice(nullptr,			// どのビデオアダプタを使用するか？既定ならばnullptrで
		D3D_DRIVER_TYPE_HARDWARE,		// ドライバのタイプを渡す。ふつうはHARDWARE
		nullptr,						// 上記をD3D_DRIVER_TYPE_SOFTWAREに設定しないかぎりnullptr
		D3D10_CREATE_DEVICE_BGRA_SUPPORT,//D3D11_CREATE_DEVICE_DISABLE_GPU_TIMEOUT,								// 何らかのフラグを指定する。（デバッグ時はD3D11_CREATE_DEVICE_DEBUG？）
		nullptr,						// デバイス、コンテキストのレベルを設定。nullptrにしとけばOK
	...
	if (FALSE(hr))
	{
		MessageBox(nullptr, L"デバイス作成失敗", L"エラー", MB_OK);
	}
	IDXGIDevice* d;
	hr=pDevice->QueryInterface(__uuidof(IDXGIDevice), (void**)&d);
	if (FALSE(hr))
	{
		MessageBox(nullptr, L"インターサーフェス取得失敗", L"エラー", MB_OK);
	}
	//アダプター取得
	hr=d->GetAdapter(&pAdapter);
	if (FALSE(hr))
	{
		MessageBox(nullptr, L"アダプター取得失敗", L"エラー", MB_OK);
	}
	//アダプターからファクトリー取得
	hr=pAdapter->GetParent(__uuidof(IDXGIFactory1), reinterpret_cast<void**>(&pFactory));
	if (FALSE(hr))
	{
		MessageBox(nullptr, L"ファクトリー取得失敗", L"エラー", MB_OK);
	}
	//スワップチェイン作成
	hr=pFactory->CreateSwapChain(pDevice, &scDesc, &pSwapChain);
	if (FALSE(hr))
	{
	...
	if (FALSE(hr))
	{
		MessageBox(nullptr, L"デプスステンシルビュー作成失敗", L"エラー", MB_OK);
	}
...
HRESULT Direct3D::InitOffScreen()
{
	//テクスチャの設定
	D3D11_TEXTURE2D_DESC offTextureDesc;
	memset(&offTextureDesc, 0, sizeof(offTextureDesc));
	offTextureDesc.Width = screenWidth;
	offTextureDesc.Height = screenHeight;
	offTextureDesc.MipLevels = 1;
	offTextureDesc.Format = DXGI_FORMAT_R8G8B8A8_UNORM;
	offTextureDesc.SampleDesc.Count = 1;
	offTextureDesc.Usage = D3D11_USAGE_DEFAULT;
	offTextureDesc.ArraySize = 1;
	offTextureDesc.BindFlags = D3D11_BIND_RENDER_TARGET | D3D11_BIND_SHADER_RESOURCE;
	offTextureDesc.CPUAccessFlags = 0;
	
	//テクスチャ作成
	pDevice->CreateTexture2D(&offTextureDesc, 0, &pOffScreenTexture);

	//シェーダーリソースビュー設定
	D3D11_SHADER_RESOURCE_VIEW_DESC offSRVDesc;
	ZeroMemory(&offSRVDesc, sizeof(offSRVDesc));
	offSRVDesc.Format = DXGI_FORMAT_B8G8R8A8_UNORM;
	offSRVDesc.ViewDimension = D3D11_SRV_DIMENSION_TEXTURE2D;
	offSRVDesc.Texture2D.MostDetailedMip = 0;
	offSRVDesc.Texture2D.MipLevels = offTextureDesc.MipLevels;

	//シェーダーリソースビュー作成
	pDevice->CreateShaderResourceView(pOffScreenTexture, NULL, &pOffScreenSRV);


	//レンダーターゲットビュー作成
	pDevice->CreateRenderTargetView(pOffScreenTexture, NULL, &pOffScreenRTV);
	return E_NOTIMPL;
}
...
HRESULT Direct3D::InitShader()
{
	...
	if (FAILED(InitShaderColoSphere()))
	{
		return E_FAIL;
	}
	return S_OK;
...
HRESULT Direct3D::InitShader3D()
{
	...
	// ピクセルシェーダの作成（コンパイル）
	ID3DBlob* pCompilePS = nullptr;
	hr=D3DCompileFromFile(L"newSimple3D.hlsl", nullptr, nullptr, "PS", "ps_5_0", NULL, 0, &pCompilePS, NULL);
	assert(pCompilePS != nullptr);
	hr = pDevice->CreatePixelShader(pCompilePS->GetBufferPointer(), pCompilePS->GetBufferSize(), NULL,
...
HRESULT Direct3D::InitShaderColoSphere()
{
	HRESULT hr;
	// 頂点シェーダの作成（コンパイル）
	ID3DBlob* pCompileVS = nullptr;
	...
	assert(pCompileVS != nullptr);
	hr = pDevice->CreateVertexShader(pCompileVS->GetBufferPointer(), pCompileVS->GetBufferSize(), NULL,
		&shaderBundle[(int)SHADER_TYPE::SHADER_COLORSPHERE].pVertexShader);
	if (FAILED(hr))
	{
		SAFE_RELEASE(pCompileVS);
		MessageBox(nullptr, L"頂点シェーダの作成に失敗", L"エラー", MB_OK);
	...
	hr = pDevice->CreateInputLayout(layout, sizeof(layout) / sizeof(D3D11_INPUT_ELEMENT_DESC), pCompileVS->GetBufferPointer(),
		pCompileVS->GetBufferSize(), &shaderBundle[(int)SHADER_TYPE::SHADER_COLORSPHERE].pVertexLayout);
	if (FAILED(hr))
	{
		SAFE_RELEASE(pCompileVS);
		MessageBox(nullptr, L"頂点インプットレイアウトの設定に失敗", L"エラー", MB_OK);
	...
	// ピクセルシェーダの作成（コンパイル）
	ID3DBlob* pCompilePS = nullptr;
	hr = D3DCompileFromFile(L"ColorSphere.hlsl", nullptr, nullptr, "PS", "ps_5_0", NULL, 0, &pCompilePS, NULL);
	assert(pCompilePS != nullptr);
	hr = pDevice->CreatePixelShader(pCompilePS->GetBufferPointer(), pCompilePS->GetBufferSize(), NULL,
		&shaderBundle[(int)SHADER_TYPE::SHADER_COLORSPHERE].pPixelShader);
	SAFE_RELEASE(pCompilePS);
	if (FAILED(hr))
	{
		MessageBox(nullptr, L"ピクセルシェーダの作成に失敗", L"エラー", MB_OK);
...
ID3D11RenderTargetView* Direct3D::GetRenderTargetView()
{
...
IDXGIFactory1* Direct3D::GetFactory()
{
	return pFactory;
}
IDXGIAdapter* Direct3D::GetAdapter()
{
	return pAdapter;
}
